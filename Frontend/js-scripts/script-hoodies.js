const urlHoodies = `http://127.0.0.1:8080/api/productos/get/hoodies`; // Ajusta URL

async function renderHoodies() {
  ...
  try {
    const response = await fetch(urlHoodies);
    if (!response.ok) throw new Error('Error al cargar hoodies');
    const hoodies = await response.json();
    ...
    hoodies.forEach(hoodie => {
      ...
      let subcat = hoodie.idSubCategoria.nombre.toLowerCase();
      if (subcat === 'series/comics') subcat = 'series';
      colDiv.setAttribute("data-category", subcat);
      colDiv.setAttribute("data-id", hoodie.idProducto);

      const badgeHTML = hoodie.tag ? `<span class="badge-tag">${hoodie.tag}</span>` : "";

      colDiv.innerHTML = `
        <div class="gorra-card-premium h-100">
          <div class="card-img-container">
            ${badgeHTML}
            <img src="${hoodie.imagen1}" class="img-front" alt="${hoodie.nombreProducto} Frontal" onerror="this.src='https://via.placeholder.com/400x400?text=No+Image'">
            <img src="${hoodie.imagen2}" class="img-back" alt="${hoodie.nombreProducto} Trasera" onerror="this.src='https://via.placeholder.com/400x400?text=No+Image'">
          </div>
          <div class="card-body-custom text-center">
            <h5 class="card-title-custom">${hoodie.nombreProducto}</h5>
            <p class="card-text-custom">${hoodie.descripcion}</p>
            <div class="price-box">$${hoodie.precio} MXN</div>
  ...
  if (productsGrid) {
    productsGrid.addEventListener("click", async (e) => {
      ...
      try {
        const response = await fetch(urlHoodies);
        const hoodies = await response.json();
        const producto = hoodies.find(p => p.idProducto === productId);
        if (producto) {
